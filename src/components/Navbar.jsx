...
const Navbar = () => {
    ...
    // --- CHATS ---
    const [chatProyectos, setChatProyectos]         = useState(0);
    ...
    // Cerrar panel de chats al hacer clic fuera
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (chatsRef.current && !chatsRef.current.contains(e.target)) {
                setPanelChatsAbierto(false);
            }
        };
        document.addEventListener('mousedown', handleClickOutside);
                ...
                <div className="hidden md:flex md:items-center md:space-x-8">
                    {perfilBloqueado ? (
                        ...
                        <>
                            ...
                            {user && (
                                <Link to="/mis-proyectos" className="text-sm font-medium text-gray-700 hover:text-black transition-colors">
                                    Mis proyectos
                                </Link>
                            )}
                        </>
                ...
                <div className="hidden md:flex md:items-center gap-4">
                    {user ? (
                        <>
                            ...
                            {/* Ícono de chats */}
                            <div ref={chatsRef} className="relative">
                                <button
                                    type="button"
                                    onClick={() => {
                                        setPanelChatsAbierto((v) => !v);
                                        setPanelNotifAbierto(false);
                                    }}
                                ...
                                >
                                    ...
                                    {chatProyectos > 0 && (
                                        <span className="absolute -top-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-blue-500 text-[10px] font-bold text-white ring-2 ring-white">
                                            {chatProyectos > 9 ? '9+' : chatProyectos}
                                        </span>
                                    )}
                                </button>
                                {panelChatsAbierto && (
                                    <ChatsPanel
                                        userId={user.id}
                                        onChatCountChange={(count) => setChatProyectos(count)}
                                        onClose={() => setPanelChatsAbierto(false)}
                                    />
                                )}
                            </div>
                                        ...
                                        >
                                            ...
                                            {chatProyectos > 0 && (
                                                <span className="ml-auto text-xs font-semibold text-blue-600">
                                                    {chatProyectos} {chatProyectos === 1 ? 'proyecto' : 'proyectos'}
                                                </span>
                                            )}
                                        </button>
                                        {panelChatsAbierto && (
